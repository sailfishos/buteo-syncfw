...
:   iDays(aSource.iDays),
    ...
    iExtendedInterval(aSource.iExtendedInterval),
...
:   d_ptr(new SyncSchedulePrivate())
{
    ...
    d_ptr->iExtendedInterval = aRoot.attribute(ATTR_EXTENDED_INTERVAL);
...
bool SyncSchedule::operator==(const SyncSchedule &aRhs)
{
    ...
    if (d_ptr->iRushDays != aRhs.d_ptr->iRushDays)
        ...
        return false;	    
    else if (d_ptr->iExtendedInterval != aRhs.d_ptr->iExtendedInterval)
        return false;
...
QDomElement SyncSchedule::toXml(QDomDocument &aDoc) const
{
    ...
    root.setAttribute(ATTR_EXTENDED_INTERVAL, d_ptr->iExtendedInterval);
...
QString SyncSchedule::extendedInterval() const
{
    return d_ptr->iExtendedInterval;
}

void SyncSchedule::setExtendedInterval(const QString &aExtendedInterval)
{
    d_ptr->iExtendedInterval = aExtendedInterval;
}
...
QDateTime SyncSchedule::nextSyncTime(const QDateTime &aPrevSync) const                                 
{
    ...
    else if (d_ptr->iInterval > 0 || !d_ptr->iExtendedInterval.isEmpty())
    {
        ...
        if (!d_ptr->iExtendedInterval.isEmpty()) {
            LOG_DEBUG("Sync extended interval defined as" << d_ptr->iExtendedInterval
                      << "with time:" << d_ptr->iTime);

            if (d_ptr->iExtendedInterval == SYNC_SCHEDULE_EXTENDED_INTERVAL_MONTH) {
                nextSync.setDate(reference.date().addMonths(1));
                nextSync.setTime(d_ptr->iTime);

                if (QDateTime::currentDateTime().secsTo(nextSync) < 0) {
                    LOG_DEBUG("Extended interval is in the past, so sync now");
                ...
                date.setDate(date.year(), date.month(),
                             d_ptr->iExtendedInterval == SYNC_SCHEDULE_EXTENDED_INTERVAL_MONTH_START ? 1 : date.daysInMonth());
                nextSync.setDate(date);
                nextSync.setTime(d_ptr->iTime);
                if (QDateTime::currentDateTime().secsTo(nextSync) < 0) {
                    LOG_DEBUG("Extended interval to" << nextSync
                              << "is in the past, so use date for next month instead");
                    // The next sync time has passed, so it must be scheduled for next month instead.
...
bool SyncSchedule::isSyncScheduled(const QDateTime &aActualDateTime, const QDateTime &aPreviousSyncTime) const
{
    ...
    // If sync schedule is defined by rush, check that rush is enabled for aDateTime
    if (rushEnabled() && d_ptr->iRushInterval > 0 && d_ptr->isRush(aActualDateTime)) {
        return true;
    }

    if (!scheduleEnabled()) {
        LOG_DEBUG("Scheduled by interval: schedule is disabled");
        return false;
    }
    ...
    QDateTime nextScheduledDateTime = nextSyncTime(aPreviousSyncTime);
    if (nextScheduledDateTime.isValid()) {
        if (aActualDateTime < nextScheduledDateTime.addSecs(5 * 60)
                && aActualDateTime > nextScheduledDateTime.addSecs(-5 * 60)) {
            LOG_DEBUG("Sync is scheduled, as next sync" << nextScheduledDateTime.toString()
                      << "is within 10 minutes of" << aActualDateTime.toString());
            return true;
        } else {
            LOG_DEBUG("Sync is not yet scheduled, next sync is" << nextScheduledDateTime.toString()
                      << "compared to given"
                      << aActualDateTime.toString());
        }
    }
